 ...
 private async void TextControl_DragEnter(object sender, DragEventArgs e)
 {
  try
  {
	if (e.DataView.Contains(StandardDataFormats.Text) && e.AllowedOperations != DataPackageOperation.None)
	{
	 ...
	 string type;
	 if (e.Modifiers == DragDropModifiers.Control) { e.AcceptedOperation = DataPackageOperation.Copy; type = "Paste"; }
	 else if (e.DataView.RequestedOperation == DataPackageOperation.Move) { e.AcceptedOperation = DataPackageOperation.Move; type = "Move"; }
	 else { e.AcceptedOperation = DataPackageOperation.Copy; type = "Paste"; }
	 e.DragUIOverride.Caption = $"{type}: {await e.DataView.GetTextAsync()}";
	 e.DragUIOverride.IsContentVisible = false;
	 IsFocused = true;
	 tempFocus = !dragStarted;
	 CursorPlace = IsSelection && place >= Selection.VisualStart && place < Selection.VisualEnd ? Selection.End : place;
	}
 ...
 private async void TextControl_Drop(object sender, DragEventArgs e)
 {
  try
  {
	if (e.DataView.Contains(StandardDataFormats.Text))
	{
	 ...
	 e.Handled = true;
	 if (tempFocus) { tempFocus = false; dragStarted = false; IsFocused = false; }
	 else Focus(FocusState.Pointer);
	}
  }
  catch (Exception ex) { ErrorOccured?.Invoke(this, new ErrorEventArgs(ex)); }
 }
